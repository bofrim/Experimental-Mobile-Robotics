...
class GameController(object):
    ...
    def joy_callback(self, message):
        ...
        for index, callback_func in enumerate(self.button_callback_map):
            rospy.loginfo("Len Buttons: %s", len(self.button_callback_map))
            rospy.loginfo("%s button's valud: %s", index, message.buttons[index])
...
if __name__ == "__main__":
    ...
    gc.x_button_callback = print_x

    joy_sub = rospy.Subscriber("joy", Joy, gc.joy_callback)
