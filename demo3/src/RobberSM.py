...
class Straight(smach.State):
    def __init__(self, rate, pub_node):
        smach.State.__init__(
            self, outcomes=["veer_left", "veer_right", "stuck", "bump", "exit"]
        )
...
class VeerLeft(smach.State):
    ...
    def execute(self, userdata):
        ...
        while not rospy.is_shutdown():
            ...
            twist = Twist()
            twist.angular.z = (
                calculate_veer_angular_velocity(
                    current_zones[INNER_RIGHT],
                    current_zones[RIGHT],
                    current_zones[OUTER_RIGHT],
                )
                * self.direction
            )
...
class VeerRight(smach.State):
    ...
    def execute(self, userdata):
        ...
        while not rospy.is_shutdown():
            ...
            twist.angular.z = (
                calculate_veer_angular_velocity(
                    current_zones[INNER_LEFT],
                    current_zones[LEFT],
                    current_zones[OUTER_LEFT],
                )
                * self.direction
            )
            twist.linear.x = 0.5 - abs(twist.angular.z)
...
def main():
    ...
    with state_machine:
        ...
        smach.StateMachine.add(
            "STUCK",
            Stuck(rate, cmd_vel_pub),
            transitions={"straight": "STRAIGHT", "exit": "exit"},
        )

        smach.StateMachine.add(
            "BUMP",
            Bumped(rate, cmd_vel_pub),
            transitions={"stuck": "STUCK", "exit": "exit"},
        )
